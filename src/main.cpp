...
#if defined(NDEBUG)
# error "Expertcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x61ccab01e910a0a1df288821d0c3f42bbb0a053645eeba9f5498c08b5a5d5c75");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Expertcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Expertcoin Signed Message:\n";
...
{
    // Expertcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	else if (nHeight < 4320) // no coin generation for 15 days ;
	{
	nSubsidy = 0 * COIN;
	}

	// code for special rewards

	else if (nHeight % 100 == 0)  // special reward for miners
	{
	nSubsidy = 500 * COIN;
	}

	else if (nHeight < 105120)  // 1 Year
	{
	nSubsidy = 20 * COIN;
	}

	else if (nHeight < 210240)  // 2 Years
	{
	nSubsidy = 40 * COIN;
	}

	else if (nHeight < 315360)  // 3 Years
	{
	nSubsidy = 50 * COIN;
	}
...
static const int64 nTargetTimespan = 10 * 30; // Expertcoin: 5 minutes
static const int64 nTargetSpacing = 2 * 30; // Expertcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Expertcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Expertcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x61ccab01e910a0a1df288821d0c3f42bbb0a053645eeba9f5498c08b5a5d5c75");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Expertcoin, an expert in blockchain technology.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("495684710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1501703484;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1349842;
        ...
        assert(block.hashMerkleRoot == uint256("0xfc0ee31b254cb8414fdf66e85d7ecb3a7f3ffb6c4369eb1fcb26a7f5ccbdfa51"));
...
unsigned char pchMessageStart[4] = { 0x4b, 0xc4, 0xba, 0x6b }; // Expertcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ExpertcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ExpertcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ExpertcoinMiner(CWallet *pwallet)
{
    printf("ExpertcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("expertcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ExpertcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ExpertcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ExpertcoinMiner, pwallet));
}
